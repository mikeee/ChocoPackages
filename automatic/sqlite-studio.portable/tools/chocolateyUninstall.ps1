...
[Environment]::GetFolderPath('Programs'), [Environment]::GetFolderPath('CommonPrograms') | ForEach-Object {
    Remove-Item -Path (Join-Path -Path $_ -ChildPath 'SQLiteStudio Portable.lnk') -ErrorAction SilentlyContinue
}

# Remove the shim
Uninstall-BinFile -Name 'SQLiteStudio'
...
Uninstall-ChocolateyZipPackage -PackageName 'sqlite-studio.portable' -ZipFileName $zipFilename
